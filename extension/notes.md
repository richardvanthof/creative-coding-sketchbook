...
use `setUniform()` to set uniforms (aka. global variables) in a shader.

read more here
https://creative-programming.netlify.app/week7/slides/3d/#/step-25
https://creative-programming.netlify.app/week7/slides/3d/#/step-24
https://p5js.org/tutorials/intro-to-shaders/
https://thebookofshaders.com/


more intresting things happen in the fragment shader.
...
function setup() {
  ...
  const frag = `
    // this is a modified verion of a shader by adam ferriss
    ...
    vec4 poly(float x, float y, float size, float sides, float rotation, vec3 col){
      ...
      // from the book of shaders shapes page https://thebookofshaders.com/07/
      // compute a circle with length(pos) and manipulate it's shape with the cos() function
      ...
      d = 1.0 - smoothstep(size*0.25, size, d);
    ...
    void main() {
      ...
      float sides = floor(mouse.x * 7.0) + 3.0; // slowly increase the sides, when it reaches 10 sides, go back down to 3
      ...
      float x = center.x + cos(1.7 * rotation) * resolution.x * 0.125;
      float y = center.y + sin(1.1 * rotation) * resolution.y * 0.125;
      // a color for the shape 
      vec3 grn = rgb(mouse.y * 200.0, 240.0, mouse.y * 200.0);
      vec3 blu = rgb(0.0, mouse.y * 100.0, (1.0 - mouse.y) * 100.0);
      ...
      poly.rgb = mix(blu, poly.rgb, poly.a);
      // render to screen
      gl_FragColor = vec4(poly.rgb, 1.0);
    }`;
  
  shapeShader = createShader(vert, frag);
}

function draw() {  
  shader(shapeShader);
  // send the resolution, mouse, and time to the shader
  // modify the mouse position and time so that it's easy to use in the shader
  shapeShader.setUniform('resolution', [width, height]);
  shapeShader.setUniform('mouse', [constrain(mouseX/width, 0, 1), constrain(mouseY/height, 0, 1)]);
  shapeShader.setUniform('time', 0.5 * millis()/1000);
